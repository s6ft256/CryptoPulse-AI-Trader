    ...
    return (
        <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                <div className="p-3 rounded-lg border border-dark-border bg-dark-surface flex flex-col items-center justify-center text-center">
                    ...
                    <div className="relative w-24 h-16 overflow-hidden">
                        ...
                        <div 
                           className="absolute top-0 left-0 w-full h-[96px] border-[12px] border-accent-green rounded-t-full border-b-0 transition-all duration-500"
                           style={{ clipPath: 'inset(0 0 50% 0)', transform: `rotate(${Math.min(0, rotation / 2)}deg)`, transformOrigin: 'bottom center' }}
                        ></div>
                        <div 
                            className="absolute top-0 left-0 w-full h-[96px] border-[12px] border-accent-red rounded-t-full border-b-0 transition-all duration-500"
...
const MacroOutlook: React.FC<{ outlook: ForexPrediction['macroOutlook'] }> = ({ outlook }) => {
    const getVixInfo = (vix: number) => {
        if (vix < 20) return { level: 'Low', description: 'Indicates low market fear and stability.', color: 'text-accent-green', bgColor: 'bg-green-500/10' };
        ...
        return { level: 'High', description: 'Signals high market fear and potential volatility.', color: 'text-accent-red', bgColor: 'bg-red-500/10' };
    };
    ...
    const getYieldSpreadInfo = (spread: 'Normal' | 'Inverting' | 'Inverted') => {
        switch (spread) {
            ...
            default: return { description: 'Indicates healthy economic expectations.', color: 'text-accent-green', bgColor: 'bg-green-500/10' };
        }
    };
    ...
    return (
        <div>
            ...
            <div className="mt-2 space-y-4">
                <div className={`p-3 rounded-lg border border-dark-border ${vixInfo.bgColor}`}>
                ...
                <div className={`p-3 rounded-lg border border-dark-border ${yieldSpreadInfo.bgColor}`}>
                    <div className="flex justify-between items-center">
...
const OrderLevelsIndicator: React.FC<{
    ...
    return (
        <div>
            ...
            <div className="flex justify-between items-baseline mt-2 p-2 rounded-md bg-dark-surface">
            ...
            <div className="mt-3 space-y-3">
                ...
                <div className="p-3 rounded-lg border border-green-500/30 bg-green-500/10 flex items-center gap-4">
                    <div className="flex-shrink-0 p-2 rounded-full text-accent-green bg-green-500/20">
                        <TakeProfitIcon className="h-6 w-6" />
                    </div>
                    <div className="flex-grow">
                        <div className="flex justify-between items-center">
                            <label htmlFor="take-profit" className="font-bold text-accent-green">Take-Profit</label>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                            <input
                                ...
                                min="0"
                                step="0.05"
                                className="w-20 bg-dark-bg border border-dark-border rounded-md text-center text-sm py-1 focus:ring-indigo-500 focus:border-indigo-500"
                            />
                ...
                {/* Stop Loss */}
                 <div className="p-3 rounded-lg border border-red-500/30 bg-red-500/10 flex items-center gap-4">
                    <div className="flex-shrink-0 p-2 rounded-full text-accent-red bg-red-500/20">
                        <StopLossIcon className="h-6 w-6" />
                    </div>
                    <div className="flex-grow">
                        <div className="flex justify-between items-center">
                            <label htmlFor="stop-loss" className="font-bold text-accent-red">Stop-Loss</label>
...
const PredictionCard: React.FC<{ 
  ...
  const trendColor = isIncrease ? 'text-accent-green' : 'text-accent-red';
        ...
        <div className="w-full">
            ...
            <div className="flex items-center gap-2 mt-2">
              <div className="w-full bg-slate-700 rounded-full h-2.5">
                  <div 
                    className={`h-2.5 rounded-full transition-all duration-500 ${isIncrease ? 'bg-accent-green' : 'bg-accent-red'}`} 
...
export const PredictionResult: React.FC<PredictionResultProps> = ({ prediction, isLoading, error, chartData, futureChartData }) => {
  ...
  return (
    <div className={`bg-dark-surface p-6 rounded-lg border border-dark-border h-full ${!hasContent ? 'flex items-center justify-center' : ''}`}>
        {renderContent()}
    </div>
  );
};
